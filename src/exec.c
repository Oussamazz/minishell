...
/*   Updated: 2020/01/18 16:01:18 by oelazzou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*find_right_path(char **args, t_node **head)
{
	...
	while (split_path[i] != NULL)
	{
		...
		if (args[0][0] != '.' && args[0][0] != 47)
		{
			...
			return (ft_ret_freetab(ft_strdup(args[0]), split_path, path_env, abs_path));
		ft_strdel(&abs_path);
		i++;
	}
	ft_treestrdel(&abs_path, &path_env, NULL);
	free_loop(split_path);
	return (NULL);
}
